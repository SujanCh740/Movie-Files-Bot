...
async def send_all(bot, userid, files, ident, chat_id, user_name, query):
    ...
    # Use secondary bot for delivery if enabled
    from lazybot import multi_clients
    delivery_bot = multi_clients.get(1) if USE_SECONDARY_BOT and 1 in multi_clients else bot
    ...
    try:
        if ENABLE_SHORTLINK:
            for file in files:
                ...
                await delivery_bot.send_message(chat_id=userid, text=f"<b>Hᴇʏ ᴛʜᴇʀᴇ {user_name} 👋🏽 \n\n✅ Sᴇᴄᴜʀᴇ ʟɪɴᴋ ᴛᴏ ʏᴏᴜʀ ғɪʟᴇ ʜᴀs sᴜᴄᴄᴇssғᴜʟʟʏ ʙᴇᴇɴ ɢᴇɴᴇʀᴀᴛᴇᴅ ᴘʟᴇᴀsᴇ ᴄʟɪᴄᴋ ᴅᴏᴡɴʟᴏᴀᴅ ʙᴜᴛᴛᴏɴ\n\n🗃️ Fɪʟᴇ Nᴀᴍᴇ : {title}\n🔖 Fɪʟᴇ Sɪᴢᴇ : {size}</b>", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📤 Dᴏᴡɴʟᴏᴀᴅ 📥", url=await get_shortlink(chat_id, f"https://telegram.me/{temp.SEC_U_NAME or temp.U_NAME}?start=files_{file.file_id}"))]]))
        else:
            for file in files:
                    ...
                    if f_caption is None:
                        f_caption = f"{title}"
                    await delivery_bot.send_cached_media(
                        chat_id=userid,
