...
async def get_shortlink(chat_id, link, userid=None):
    ...
    if settings.get('is_shortlink'):
        if URL == "api.shareus.io":
            ...
            try:
                ...
                res_link = await shortzy.convert(link)
            except Exception as e:
                logger.error(e)
                res_link = link
    
    if userid:
        return await get_webapp_verify_link(userid, res_link)
    return res_link
